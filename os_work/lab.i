...
void push(PCB** head, PCB* e) {
    PCB* p = *head;
    while(p->next) {
        if(e->priority < p->priority && p->next->priority < e->priority) {
            e->next = p->next;
            p->next = e;
            return;
        } else {
...
void print1(PCB* p) {
    ...
    while(p) {
        printf("name:%scputime:%ld\tneedtime:%ld\tpriority:%hu\tstatus:%s\n", p->name, p->waiting, p->need_time, p->priority, get_status(p->status));
        p = p->next;
    }
}
...
# 118 "lab.c"
                 ;
    scanf("%s%ld%hd", p->name, &p->need_time, &p->priority);
    p->waiting = p->need_time;
    p->status = R;
    push(&begin, p);
}

int main(int argc, char* argv[]) {
    freopen("/home/freaver/CODE_C/input.txt", "r", 
# 126 "lab.c" 3 4
                                                  stdin
# 126 "lab.c"
                                                       );
    freopen("/home/freaver/CODE_C/output.txt", "w", 
# 127 "lab.c" 3 4
                                                   stdout
# 127 "lab.c"
                                                         );
