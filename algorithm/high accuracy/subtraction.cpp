 ...
 * @LastEditTime: 2021-01-30 15:22:32
 */
...
int sizeJudge(char *clSubtrahend, char *crSubtrahend, int len)
{
    int i;
    for ( i = 0; i < len; i++ ) {
...
void sub(char *clSubtrahend, char *crSubtrahend, char *cas, int lenL, int lenR, int maxLen)
{
    int i;
...
void highSub(char *clSubtrahend, char *crSubtrahend)
{
    char cas[500] = {0};
...
{
    char clSubtrahend[500] = {0};
    char crSubtrahend[500] = {0};
    scanf("%s%s", clSubtrahend, crSubtrahend);
    highSub(clSubtrahend, crSubtrahend);
    printf("%s", clSubtrahend);
    return 0;
}
